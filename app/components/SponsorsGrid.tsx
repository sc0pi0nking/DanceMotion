...
interface SponsorCardProps {
  ...
  onImageLoad: (sponsorId: string, event: React.SyntheticEvent<HTMLImageElement>) => void;
}

function SponsorCard({ sponsor, isPortrait, categoryLabel, onImageLoad }: SponsorCardProps) {
  return (
    <article
      className="group h-full rounded-3xl overflow-hidden border transition-all duration-200 hover:-translate-y-1 focus-within:-translate-y-1"
      style={{
    ...
    >
      ...
      <div
        className={`relative overflow-hidden flex items-center justify-center px-2 ${
          isPortrait ? 'h-64 md:h-72' : 'h-52 md:h-56'
        }`}
        style={{
      ...
      >
        ...
        {sponsor.logo_url ? (
          <img
            ...
            loading="lazy"
            decoding="async"
            onLoad={(event) => onImageLoad(sponsor.id, event)}
          ...
          <div className="hidden md:grid grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
            {filteredSponsors.map((sponsor) => (
              <SponsorCard
                key={sponsor.id}
                sponsor={sponsor}
                isPortrait={!!isPortraitImage[sponsor.id]}
                categoryLabel={getCategoryLabel(sponsor.category)}
